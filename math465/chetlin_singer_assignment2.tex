...
\end{problem}
\begin{proof}
  To begin with, note that since \( \left| \alpha(t) \right| \) obtains a maximum when \( t = t_0 \), it must be that \( \frac{\mathrm{d} }{\mathrm{d} t}\left| \alpha(t_0) \right| = 0 \).
  That is
  \begin{align}
  \label{eq:18}
    \frac{\mathrm{d} }{\mathrm{d} t} \sqrt{\langle\alpha, \alpha\rangle} &= \frac{\langle \alpha^{\prime}, \alpha \rangle + \langle\alpha, \alpha^{\prime} \rangle}{2 \sqrt{\langle\alpha, \alpha\rangle}} \\
    &= \frac{\langle\alpha^{\prime}, \alpha \rangle}{\sqrt{\langle\alpha, \alpha\rangle}} = 0.
  \end{align}
  In other words, \( \alpha^{\prime}(t_0) \) is orthogonal to \(\alpha(t_0)\).

  Now, \( \alpha \) is not necessarily parametrized by arc length, hence, with \( T \) denoting the \textbf{non-unit} tangent vector, the curvature \( k \) of \(\alpha\) satisfies
  \begin{align}
  \label{eq:19}
    k(s) &= \left| \frac{\mathrm{d} }{\mathrm{d} t} \frac{\alpha^{\prime}(s)}{\left| \alpha^{\prime}(s) \right|} \right| \\
         &= \left| \frac{|T(s)| \alpha^{\prime\prime}(s) - T(s) \frac{\mathrm{d} }{\mathrm{d} t} |T(s)|}{|T(s)|^2} \right| \\
    &= \frac{1}{\langle\alpha^{\prime}, \alpha^{\prime}\rangle}\left| \sqrt{\langle\alpha^{\prime}, \alpha^{\prime}\rangle}\alpha^{\prime\prime} - \frac{\alpha^{\prime}\langle \alpha^{\prime\prime}, \alpha^{\prime}\rangle}{\sqrt{\langle\alpha^{\prime}, \alpha^{\prime}\rangle}} \right|.
  \end{align}

  Now, since \( \langle\alpha^{\prime}(t_0), \alpha(t_0) \rangle = 0\), differentiating both sides shows that in fact \( \langle\alpha^{\prime\prime}, \alpha \rangle = - \langle\alpha^{\prime}, \alpha^{\prime}\rangle \).
\end{proof}
